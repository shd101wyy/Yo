...
function generateDropFunctionCode(enumType: EnumType): string {
  ...
  const needsVariantHandling = enumType.variants.some(
    (variant) =>
      variant.elements &&
      variant.elements.some((element) => typeContainsARCType(element.type))
  );

  if (!needsVariantHandling) {
  ...
  // For now, we'll implement a simple version that just calls __yo_decr_rc
  // More complex variant-specific destructuring can be added later
  return `((fn(mut(self): Self) -> unit) {
  __yo_decr_rc(self${hasDisposeFunction ? `, Self.dispose` : ""});
  ()
})`;
