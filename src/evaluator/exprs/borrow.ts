...
export function evaluateBorrow({
  ...
  for (let i = 0; i < borrowedValueExprs.length; i++) {
    ...
    if (isType0(evaluatedBorrowedValueExpr.$.type)) {
      throw formatErrorMessage({
        token: borrowedValueExpr.token,
        errorMessage: `Cannot borrow a Type, got:\n${typeToString(
          evaluatedBorrowedValueExpr.$.type
        )}`,
      });
    }

    // For closure types, ensure isClosure is defined
    const borrowedType = evaluatedBorrowedValueExpr.$.type;
    if (
      isClosureType(borrowedType) &&
      isFunctionType(borrowedType.callType) &&
      !borrowedType.callType.isClosure
    ) {
      throw formatErrorMessage({
        token: borrowedValueExpr.token,
