...
- **A `unit` field COSTS ONE BYTE (plus padding) — `sizeof(unit)` is 1, not 0.** C has no zero-sized object type, so every storage position (parameter, struct/tuple field, local, cast target) spells a `unit` as a one-byte `uint8_t` placeholder, keeping field count and order (`get_storage_type_string`, `src/codegen/utils/index.yo`). The layout model mirrors that: `get_size_of_type(.Unit)` is 8 bits (`src/types/utils.yo`), so `struct(a : i32, b : unit)` is **8** bytes, `struct(b : unit)` is **1**, and `Tuple(i32, unit)` is **8**. It has to: `sizeof` folds to a C literal and then sizes allocations that C-typed pointer arithmetic strides through, so a model that counted the field as zero made `malloc(n * sizeof(S))` short of the `n * sizeof(struct S)` addressed — an out-of-bounds heap write from ordinary safe Yo (`issues/fixed/sizeof-of-aggregate-with-unit-field-disagrees-with-emitted-c-struct.md`). **The one exception is an ENUM VARIANT field**, which the tagged-union emitter *erases*: `enum(A(x : i32, u : unit), B)` is 8 (tag + `i32`), not 12, and `Option(unit)` is tag-only at 4. If you ever change the placeholder's width, or drop it at some position, change `get_size_of_type` in the same commit — those two are one rule expressed twice.
