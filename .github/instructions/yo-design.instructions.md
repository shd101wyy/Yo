...
- **`unit` is a TRUE zero-sized type — `sizeof(unit)` is 0, as in Rust, and a `unit` field costs nothing.** The emitter lays out NOTHING for it: a `unit` struct/tuple field emits no C member (`_emit_runtime_fields` / `generate_tuple_declaration`, `src/codegen/types/generation.yo`), an enum variant field is erased from the data union, `Array(unit, N)` has no `data` member, a unit local declares no variable, and a read of a unit field emits no C (`generate_field_access`). C struct members are accessed by NAME, so erasing one shifts nothing. So `struct(a : i32, b : unit)` is **4** bytes, `struct(b : unit)` is **1** (the dummy), `Tuple(i32, unit)` is **4**, `enum(A(x : i32, u : unit), B)` is 8 (tag + `i32`) and `Option(unit)` is tag-only at 4. An aggregate whose every field is `unit` gets a single one-byte `_zst_dummy` member and measures **1** — NOT 0 and NOT an empty struct: clang lays an empty struct out as 0 bytes on GNU targets but **4 bytes on the MSVC ABI** (`*-windows-msvc`, measured with `-Xclang -fdump-record-layouts`), so an empty struct is not a portable ZST and a model saying 0 under-allocated every container of such an aggregate on Windows. An enum with no non-unit variant field emits NO data union and no `data` member (an empty union is 4 bytes on MSVC too — `issues/fixed/empty-enum-data-union-is-4-bytes-on-msvc.md`). **Never rely on an empty C aggregate having size 0.** **Every consumer of a layout must skip the same fields** — the `__yo_new_` constructor's parameters and stores (`functions/constructors.yo`, `functions/declarations.yo`), the ctor call and the value-struct compound literal (`exprs/other_fn_call.yo`, `exprs/comptime_value.yo`), the tuple literal (`exprs/tuple_fn.yo`) — because `sizeof` folds to a C literal that sizes the allocations C-typed pointer arithmetic strides through, and a model/emitter disagreement in EITHER direction is a heap bug (`issues/fixed/sizeof-of-aggregate-with-unit-field-disagrees-with-emitted-c-struct.md` was the under-allocating direction). **The one place a `unit` still occupies a byte is a BY-VALUE PARAMETER** (`get_storage_type_string`, `src/codegen/utils/index.yo` — C cannot declare a `void` parameter, so it is a `uint8_t` fed `0`); that is calling convention, not layout, and `sizeof` never observes it. **Containers must special-case a zero element size** rather than divide or `realloc` by it: `ArrayList` of a ZST uses one one-byte anchor block and `SIZE_MAX` capacity (`_zst_anchor`), because `realloc(p, 0)` frees `p` on glibc and the Windows CRT. History: this arm went 0 → 1 → 0 (`issues/fixed/unit-should-be-a-true-zero-sized-type-like-rust.md`).
...
- **Comptime.** `unwrap` is not comptime-evaluable. The comptime spellings are `comptime_unwrap` / `comptime_unwrap_err`, and they are legal everywhere. A `__yo_panic` reached by a LIVE compile-time execution is a compile error, `compile-time panic: <msg>` (`evaluate_panic`). A body is live when every argument is known and no untaken arm of an enclosing comptime body contains the call (`FuncOrAsyncBlockCtx.ctfe_live` / `EvalContext.ctfe_untaken_arm_depth`). Do not key a new comptime diagnostic on `ctx.is_executing && !ctx.is_validating_function_definition` alone: a comptime call inside a runtime function body runs in a validating context, and a gate built that way misses it (measured on the first version of this check).
...
- **Escaped unwinds are never lost, and observed aborts are silent.** An unwind that aborts an async task is *registered*, not printed. Any read of the Aborted state observes it silently: `JoinHandle.await` → `.None`, an awaiter's `io.await` (the escape propagates and the awaiter is registered in turn), `JoinHandle.state()` / `is_finished()`, or `io.state`. A task nobody observed prints `unhandled effect unwind aborted an async task that was never awaited`, once, either when its last reference is released or when the program body returns. `JoinHandle.abort()` cancellation never registers. An unwind escaping `main` prints `unhandled effect unwind escaped to top level` and aborts. A test-batch `main` clears the flag instead, as the test-runner boundary (`issues/fixed/an-awaited-task-abort-is-reported-as-unhandled.md`).

## Parallelism rules (`plans/reference/PARALLELISM_RULES.md`)

The guarantee is data-race freedom for every program that compiles without `pragma(Pragma.AllowUnsafe)` and uses std's primitives. The rules a std or compiler author runs into:

- **D1, module globals.** A closure bound to a `Send` closure type (a spawn body, a pool task, a `Dyn(Trait, Send)` method) may not reach a non-`Send` module-level global, directly or through any function, local closure or `Send` trait object it calls. A value global that is written anywhere may not also be reached that way (a mutable static). Check: `function_reaches_non_send_global` (`src/evaluator/effects/mutation_summary.yo`). Use `thread_local(name)` for per-thread state, an atomic object for shared state. A pragma'd std file is the audited base: its globals are under a lock (`std/log.yo`, `std/encoding/html.yo`).
- **D3, no writes through an atomic object.** No `=`, index store, or `inout` binding whose callee may write, rooted in an `Arc`/`Mutex`/`atomic(ref(...))`. The decision uses the callee's per-parameter mutation mask. A read-only `inout(self)` method (`ToString`, `Hash`) through an `Arc` stays legal.
- **D8, second-class captures.** A closure may not capture an `inout`/`ref` parameter or a control-bound value. A new rejection that fires inside a definition-time trial must be flagged with `flag_flow_violation` before it throws, or the trial swallows it.
- **D4, closure types.** A closure has a marker trait (`Send`, `Acyclic`) iff its capture struct does, wherever `where(T <: Send)` is discharged (`validate_where_constraints_for_call`).
