...
- **The two string forms have DIFFERENT escape tables, and an unknown escape is silently literal in both.** `\n \t \r \\ \" \' \0 \b \f \v` work in both; `\uXXXX` (exactly four hex digits) works ONLY in a double-quoted literal; `` \` `` and `\$` only in a template. `\u{...}` and `\xNN` exist nowhere. Measured 2026-09-11: `` `x\u0041y` `` keeps all 8 bytes while `"x\u0041y"` is `xAy`, and a malformed `"\uZZZZ"` decodes to NUL because each non-hex digit counts as 0 (`issues/unicode-escape-accepts-non-hex-digits.md`). Write a control byte with `push_byte`, never an escape.
