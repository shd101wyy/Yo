...
- **Pointer operators are the `&`-prefixed family** (generic `impl(generic(T), *(T), ...)` in the prelude, lowering to `__yo_ptr_*` builtins): identity `p &== q` / `p &!= q`, ordering `&<`, `&<=`, `&>`, `&>=`, arithmetic `p &+ n` / `p &- n` (offset by `usize`), difference `&/` (→ `isize`). **Plain `==` does NOT exist for typed pointers** (`*(u8) == *(u8)` fails overload resolution — only `*(void)` has a plain `Eq` impl); use `&==` for pointer identity, e.g. the interned-string fast path in `std/string/string.yo`'s `==`. Comparisons are safe (no `unsafe(...)` needed); arithmetic requires `unsafe(...)`.
