...
- The download needs `tar` on the `PATH`. As of v0.2.22 the HTTP itself is spoken by the compiler through `std/http` and the compiler-emitted TLS backend — the `curl` dependency is gone (it was a shell-out in every release before that). **Windows speaks TLS through Schannel** (SSPI), which ships with the OS — nothing to install. On a unix machine without OpenSSL the remote subcommands report `TLS is unavailable in this build` with install guidance (`brew install openssl@3`, or your distribution's `libssl-dev`). Node.js and Bun are **not** required — they were only needed back when Yo shipped as an npm package.
