...
The `std/io/` directory provides the low-level async I/O foundation. Users import submodules directly — there is no barrel `index.yo`:
...
- **Barrel re-export removed**: `std/io/index.yo` removed to avoid naming conflicts. Users import submodules directly.
...
- **macOS getnameinfo NI_NUMERICHOST wrong value**: Added platform-aware `NI__*` constants to `std/io/socket.yo`.
