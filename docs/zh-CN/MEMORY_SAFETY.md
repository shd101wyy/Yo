...
这是安全模式规则 D7：调用不得抹除类型所携带的失败信息。它并不是禁止中止 —— `panic(...)` 和 `assert(...)` 仍然合法（刻意的中止是作者响亮的表态，`assert` 也是测试基础设施的基础），值级别的陷阱（如边界检查的索引）也仍然合法（它们的失败是类型从未携带的*值*前提条件，属于算术语义工作的范畴，而非本规则）。
