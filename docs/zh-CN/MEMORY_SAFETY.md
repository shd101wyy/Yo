...
这是安全模式规则 D7（`plans/SAFE_MODE.md`）：调用不得抹除类型所携带的失败信息。它并不是禁止中止 —— `panic(...)` 和 `assert(...)` 仍然合法（刻意的中止是作者响亮的表态，`assert` 也是测试基础设施的基础），值级别的陷阱（如边界检查的索引）也仍然合法（它们的失败是类型从未携带的*值*前提条件，属于算术语义工作的范畴，而非本规则）。
...
对于"失败即结果"的风格，有 checked 方法：`i32(1).checked_add(i32(2))` 是 `.Some(3)`，`i32.MAX.checked_add(i32(1))` 是 `.None`。饱和变体直接钳制（`saturating_add`），而且**浮点→整数的转换会饱和** —— 超出范围的值变成目标类型的 MIN/MAX，`NaN` 变成 `0` —— 而不是未定义行为。
