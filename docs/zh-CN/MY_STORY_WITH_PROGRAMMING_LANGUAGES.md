...
在这个过程中，我还尝试过用有栈协程（stackful coroutine）来实现 `Yo` 的异步运行时。我实现了它们并让它们跑起来了，但最终决定切换回无栈协程（stackless coroutine）。有栈协程引入了额外的开销——每个协程都需要自己的栈，而确定合适的栈大小是个棘手的问题。太小了容易栈溢出，太大了又浪费内存。无栈协程将异步函数编译为状态机，更符合 `Yo` 的设计理念：零开销、内存使用确定性强，而且与 C 代码生成流水线无缝集成。
