...
> **状态：** 上文描述的回收器在每次回收时都会扫描**整个**被跟踪对象集合。在分配密集、
> 绝大多数对象存活且环极少的工作负载下（最典型的是编译器自举编译自身），这会成为
> 主要开销。下面的优化是计划中的修复；当前的临时方案是一个环境变量开关。
...
### 计划修复：Bacon-Rajan 可能根集（O(根集) 而非 O(堆)）

正确的修复是 **Bacon-Rajan 同步环回收**算法。关键观察：只有引用计数被**递减到非零
值**的对象才可能是垃圾*环*的根（"可能根"）。因此回收器不应扫描每个被跟踪对象，而应
只处理**可能根缓冲区**及其可达子图：
...
这使每次回收为 **O(可能根 + 其可达子图)** 而非 O(所有被跟踪对象)，因此环极少的工作
负载（编译器）回收开销很低，回收器可默认开启而无需环境变量覆盖。这是一项关乎正确性
的改动（涉及释放路径），在落地前需在 AddressSanitizer 下针对环回收测试
（`tests/cycle_collector.test.yo`、`tests/codegen-bootstrap/*_self_cycle.yo`、
`ref_enum_cycle.yo`）验证。
