...
> **内存：** 构建 Yo 本身（无论是 `--from-source`，还是在代码仓库中运行 `yo build`），
> 在 v0.2.43 或更新的编译器下，一台 **8 GB** 内存的机器就足够。完整构建编译器的峰值约为
> 4.3 GiB（在 Linux 上测得，包含 C 编译器），CI 在每次变更时都会在 8 GB、无 swap 的限制下
> 构建它。更早的版本约需 10 GB。编译你自己的程序所需的内存要少得多。
