...
| `CompilationTarget.X86_64_Linux_Musl`   | `x86_64-linux-musl`   | Linux x86-64（静态 musl） |
...
Yo 通过目标三元组支持交叉编译。可以在 `build.yo` 中或命令行上指定目标：
...
| `x86_64-linux-musl`   | Linux x86-64（静态 musl） |
