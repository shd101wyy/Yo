...
Options:
  ...
  --watch                源码变化时重新构建（产物在同一进程内重新编译）
  --poll-ms <ms>         监听轮询间隔（默认 200）
  --max-rss-mb <N>       仅监听模式：常驻进程超过 N MB 时重启
  --profile              分阶段耗时；配合 --watch 时每轮额外输出一行
```
...
`yo build --watch` 让一个编译器进程常驻。它先把请求的步骤跑一遍，然后轮询这些
编译读取过的文件（`--poll-ms`，默认 200 毫秒），每次变化都重新运行这些步骤。
一轮监听比一次全新的 `yo build` 便宜，原因有两点：

- **产物在同一进程内重新编译。** 普通构建为每个产物派生一个 `yo compile` 子进程，
  每个子进程都要从头求值标准库和项目模块。监听轮次则在常驻进程内、基于已经求值过的
  模块来编译产物；只有这次编辑触及的定义（以及依赖它们的定义）会被重新求值。
  C 编译器仍然以子进程运行。
- **输入戳记依然有效。** 记录的输入没有变化的产物会像上一节那样被跳过，所以一轮
  只为这次编辑波及的产物付费。

`--profile` 每轮打印一行 `profile: watch round N <ms> rss=<MB>MB`；第 1 轮是冷启动
基线。常驻进程的内存会随轮次累积。`--max-rss-mb N` 会在常驻集超过 N MB 之后的第一个
轮次边界干净地重启它（同一个进程 id，全新的镜像）；默认永不重启。

### 分块 C 输出

`yo compile` 默认只输出一个 C 文件。`--emit-chunks <n|auto>` 把输出的 C 切成 `n` 个
翻译单元并行编译（`--jobs <n>`；`auto` 根据输出大小和核心数选择数量，`YO_JOBS`
可以覆盖）。没有显式给出 `--jobs` 时，任务数还会被限制，使同时运行的 C 编译器
占用不超过物理内存的一半（Linux 上若 cgroup 限制更低则以它为准）；
`YO_ASSUME_MEMORY_MB=<n>` 设置这个上限所用的内存数值。每个单元在目标文件旁边都有自己的对象缓存（`<obj>.inputs-sha256`），
所以再次编译一个改动过的程序时，只有 C 发生变化的单元会重新编译：

```
chunks: 10 unit(s), 7 cached, 3 to compile (jobs=8)
```

单元默认按 C 名称分组；`--chunk-by module` 把一个模块的函数放进同一个单元，这样
一处函数体改动只弄脏一个单元。`--emit-chunks` 在任何高于 0 的 `--optimize` 级别下与 ThinLTO 配对；
`--no-chunk-lto` 可以退出。在 `-O0` 下分块并行编译比单文件快约三倍，运行时行为
完全一致；在 `-O2` 下 LTO 链接是下限，分块收益很小。`YO_BUILD_NO_CACHE=1` 同样
会禁用对象缓存。
