...
分支按源码顺序尝试，首个匹配者胜出。穷尽性按结构检查：`.Some(true), .None` 会被拒绝并报告 `Missing case: .Some(false)`；整数、浮点数和字符串需要通配符或绑定分支；带守卫的分支不算覆盖其模式。没有任何值能到达的分支是错误（`Unreachable match arm`），但完全覆盖之后的末尾 `_` 可以保留。绑定在分支内借用被匹配的值。
