...
The capture analysis (`src/evaluator/async/await_analysis.yo`) /
state-machine lowering must not mark a temp whose lifetime is bounded by the
loop-body begin-block as live-across-await; alternatively the deferred drop
must fire at the begin-block epilogue (on the local) rather than at
completion (on the never-stored slot) whenever no store to the slot was
emitted. Any fix here needs the dup/drop emit-diff gate plus an
over-cancellation canary per AGENTS.md.
