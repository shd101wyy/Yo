...
| recorded root (trait-bound accumulation) refuted                                        | `grep -o "F____Fn[A-Za-z0-9_]*"` → **exactly 1** distinct key, one `Fn(*(A))->bool` bound. Recorded hypothesis is at `issues/yo-self-hollow-root-cause-map.md:3672` and is indeed dead                                                                                                                                                                                     |
...
- The root it describes is largely the already-recorded era-split root (`issues/yo-self-hollow-root-cause-map.md`, 2026-07-31 + the 2026-07-30 imm_sorted_map ROOT CAUSE: "substitution keeps def-era ids; TS re-evaluates type exprs through the ctor memo"), plus `issues/repros/comptime-ctor-memo-split-map-insert.yo`. It reads as a rediscovery framed as new.
