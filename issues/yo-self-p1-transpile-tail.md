...
| `value.yo` | 6→2 swallowed | `Self`-not-found ×4 (378914804) + `field_labels` (8910182ad) RESOLVED. DBG_SW2 re-survey shows 2 remain: (a) `field_types` ×1 — `types.clone()` (the SECOND arg of the SAME `.Tuple(labels, types) => TypeValue.Tuple(labels.clone(), types.clone())` at definitions.yo:392, masked behind field_labels before) where `types : ArrayList(Self)` has the RECURSIVE-ENUM element `TypeValue` (vs `labels`'s concrete `String`) → still Type(1); receiver-arg-type fix did NOT clear it = recursive-enum-element + Self-substitution intersection (deeper). (b) `and` ×1 — `name.starts_with("Box(")` at guards.yo:561 → non-bool. KEY: `starts_with` has a DEFAULT ARG (`starts_with(self, prefix, (position : usize) ?= 0)`, std/string/string.yo:842) and the call OMITS it, so this is most likely a DEFAULT-ARG-at-def-time-eval result-type bug (the func-id default-args side-table, see [[yo-self-default-args-side-table]] — TWO call paths), NOT a generic method-bool issue; the omitted-default call resolves to a non-bool result type. The 6 visible `// Failed to transpile` MARKERS are all `if(...)`-as-value COLLATERAL = separate OPEN issue `yo-codegen-block-rhs-drops-statements`. |
