...
specialization-input paths; whack-a-mole at each input does NOT converge (5 tried).
The CLEAN fix is at the SOURCE — resolve the shell where `types : ArrayList(Self)`
is BOUND at the destructure (match.yo arm binding), or in the recursive-enum
representation itself, so no shell ever reaches specialization. That is a focused
