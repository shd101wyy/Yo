...
`\_\_CP`probe on comptime_str args never fires for the`f`-call's String param). So
...
`issues/fixed/yo-self-comptime-str-to-string-codegen.md`). The original effect handler
with a backtick message — `raise(\`zero\`)`— now compiles + runs →`zero`/`104`matching TS (fixture`effect_handler_backtick.yo`, corpus 72/72). So synchronous
...
template arg evaluated against an expected `String`type** that throws (the`("zero".to_string)()`desugaring under expected-String, during def-eval).`String.from("zero")`with the same expected String does NOT throw. NARROW edge —
...
String. NEXT: probe the arg-eval at check_if line ~438 (move the marker BEFORE`evaluate_expression_raw`) to capture the throw, then fix the backtick/to_string
