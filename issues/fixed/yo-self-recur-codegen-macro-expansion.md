...
Fix (4 files): a durable id-keyed `g_macro_expansions` side-table (expr*info.yo,
...
(`get_type_string: no C type name found for <struct:capture*…>`). The collection fallback
...
set during _some_ eval pass but **lost** on the IF node codegen reads.
