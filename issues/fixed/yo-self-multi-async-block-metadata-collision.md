...
**SomeT-wrapped** type, so`type_key`hits the`.SomeT({ id })`case, never the`.FutureTraitT`case. And the collision means the two calls' Impl-SomeT result
types **share the same SomeType id** — i.e. the function-signature return type`Impl(Future(T,E))`'s SomeType is NOT freshened per call site. So both
`register_type`calls use the same key; the second (5660) overwrites the first
(5658);`t1`'s binding declaration then resolves to 5660 while `t1`'s body emits 5658.
