> **RETIRED (2026-08-06).** Superseded record — June marker-count triage; the fix was reverted and the whole marker metric was superseded by the 0-marker fixpoint.
...
- **NEW (the cost):** `((self.*).clone)()` on a _different_ type (3562); codegen `if((entry.c_name) == c_name…)`;
...
parser.yo:1099) — they were ALREADY forward-shelled by the pre-existing direct-field pre-pass, _before_
...
6 sites from warm-up-masked-SUCCESS to FAIL. In warm-up masking a masked site _transpiles correctly_ in
...
The first theory (from §3 of `plans/archive/RECURSIVE_TYPE_REPRESENTATION.md` and a root-cause
...
- It is **self-similar / recursive**: `items.clone_list()` returns `TypeVal(Comptime)` _because_ its
