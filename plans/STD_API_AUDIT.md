...
The remaining D4 steps (PRs 5-9) are unchanged and still in
...
- **O1 (D4)**: **DECIDED — byte-indexed, matching Rust.** `len()` = bytes O(1),
  ...
  deleted (measured dead). What remains is the `ImmString` rename (PR 5),
  regex's `index()` (PR 6), the comptime basis (PR 7), the docs sweep (PR 8)
  and the decoder dedup (PR 9).
