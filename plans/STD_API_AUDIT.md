...
| C54 | **Two specializations of `-> Impl(Future(R))` collide** — the caller cast both futures to the second spec's struct (FIXED 2026-08-29), the first spec's async body renders its `body(..)` result as the second spec's `R` (FIXED 2026-08-30, #367), and the E variant — an io.async closure's bundle-param slot rendering the shared forall E through the global last-winner (the develop ASan overread) (root-caused 2026-08-31; fix pending evaluator-side) | **OPEN (E variant)** — `Mutex.with_lock` restored; the E half is root-caused with the correct fix identified (register the closure's inferred concrete param at def-eval) — issues/future-wrapper-return-shared-across-specializations.md, issues/asan-stack-overread-set-effect-batch-selftest.md |
