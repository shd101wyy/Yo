...
| C16 | ~~A trait `?=` DEFAULT that awaits `Self.<async method>` is never monomorphized — the emitted C function is hollow (`void*` params, body of `// Failed to transpile` comments) while `check` and `compile` are both green. Blocks D5's default methods (`read_to_end`, `write_all`, `lines()`)~~ **FIXED 2026-08-26.** The diagnosis in the row was WRONG on the mechanism: the default IS monomorphized per implementor (`impl.yo`'s per-impl materialization mints a fresh body clone + func_id). What was missing is that the materialization evaluated that body under the IMPL's ambient `EvalContext` instead of a FUNCTION-BODY one, so the body had no expected return type — and `io.async`'s Step 6b binds the effect-bundle generic `E` FROM that expected `Impl(Future(T, E))`. With `E` unbound, `e.io.await(...)` failed to resolve, the def-eval-wall swallowed it silently, and codegen had no ExprInfos. `Self` is NOT required to trigger it (an await of a FREE async fn in the same default hollows identically). Same root cause made `return(...)` in ANY trait default throw `Variable "return" not found`. Fix: build the body ctx with `create_function_body_evaluation_context` (moved to `evaluator/context.yo` to break the import cycle). Test: `tests/async_trait_default_await.test.yo`. Issue: issues/fixed/trait-default-awaiting-self-async-method-emits-hollow-fn.md | compiler |
...
**Compiler blocker C16 is CLEARED (2026-08-26) — but D5 is only PARTLY
unblocked.** An async trait `?=` default that awaits — `Self.read` or anything
else — now evaluates and emits a real per-implementor state machine instead of a
hollow `abort()` body, including the loop-until-done shape `read_to_end` needs
(verified 2026-08-26). See
...
- **C21** makes the emitted C for a two-implementor async default trip
  ...
  reason that warning is on is that this class has silently miscompiled before.
...
- **O1 (D4)**: **DECIDED — byte-indexed, matching Rust.** `len()` = bytes O(1),
  ...
  a panic for a non-boundary index. **Regex's `index()` (PR 6) landed
  2026-08-26** — a byte index, six conversion walks deleted (release-note
  item). What remains is `imm.String` (PR 4), the `ImmString` rename (PR 5),
  the comptime basis (PR 7), the docs sweep (PR 8) and the decoder dedup
  (PR 9).
