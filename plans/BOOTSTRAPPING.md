...
### Strategy: 1-to-1 TypeScript → Yo Porting (current focus)

For now, the only acceptable porting style is a **strict, faithful 1-to-1 translation** of the TypeScript codebase under `src/` into Yo source files under `yo-self/`. Concretely:

- **Every TypeScript file maps to exactly one Yo file** following the table in [Port Strategy: 1:1 TypeScript → Yo](#port-strategy-11-typescript--yo). Function names, struct/class names, parameter order, and even comments should match the TS source as closely as Yo's syntax allows.
- **No simplifications, no shortcuts, no architectural redesigns.** If a TypeScript function does X, the Yo function must also do X — even if a "smarter" Yo design exists. The goal is to get a correct self-hosted compiler first; refactoring is a separate later phase.
- **No prototype shortcuts.** The legacy prototype files (`yo-self/codegen/exprs.yo`, `yo-self/codegen/driver.yo`, the old `yo-self/evaluator/eval.yo`) are scaffolding that exists only to keep tests green during the porting transition. They will be **deleted** once their TypeScript counterparts (`src/codegen/exprs/*.ts`, `src/codegen/index.ts`, `src/evaluator/exprs/*.ts`) are fully ported.
  - `driver.yo` in particular has **no TypeScript counterpart** (`src/driver.ts` does not exist) and is purely a transitional bootstrap shim. It must be removed once `yo-self/codegen/index.yo` (mirroring `src/codegen/index.ts`) is wired up end-to-end.
- **Prefer porting work over feature work.** When a test fails because a feature is missing, the right fix is almost always to port the corresponding TypeScript code path, not to hand-roll a one-off Yo implementation.

### Final goal: pass `./tests/` with the self-hosted compiler

The bootstrapping is considered **complete** when `yo-self/yo-self-bin` (the binary built from `yo-self/main.yo`) can run the entire `./tests/` integration test suite and pass it with the same results the TypeScript-built `yo-cli` produces today. Concretely:

```bash
# Build the self-hosted compiler with the TS-built yo-cli
./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin

# Run the full integration test suite using the self-hosted binary as the
# compiler under test (instead of the TS-built yo-cli)
./yo-self/yo-self-bin test ./tests/ --bail
```

When that command succeeds end-to-end, the self-hosted compiler is at parity with the TypeScript implementation and we can begin retiring `src/`. Until then, every porting decision must be evaluated against the question: **"does this make the self-hosted compiler closer to passing `./tests/`?"**
...
| `src/codegen/exprs/*.ts`                            | `yo-self/codegen/exprs/*.yo`                            | 🔄 In progress (Phase 4Z-split + Phase 13at/13au/13av/13aw/13ax/13ay/13az/13ba metadata-aware split: `expr.yo`, `gc.yo`, `sizeof.yo`, `consume.yo`, `async_completion.yo`, `parallelism.yo` (`__yo_thread_set_maximum_threads`), `inline_fns.yo` (operators, ptr/slice ops, randomness, sleep, noop, decr_rc, maybe_uninit_assume_init), `typeid.yo`, `panic.yo`, `open.yo`, `binding.yo`, `tuple_fn.yo`, `array_fns.yo`, `recur.yo`, `iso.yo`, `while.yo`, `and_or.yo`, and `begin.yo` ✅ ported and wired as 1-to-1; remaining handlers still inline in `exprs.yo`) |
...
The current `yo-self/evaluator/eval.yo` and `yo-self/codegen/driver.yo` (along with `yo-self/codegen/exprs.yo`) are **prototype scaffolding** — they cover only a small fraction of the TypeScript evaluator and codegen and exist solely to keep the bootstrap binary functional during the porting transition. They will be **deleted** once the corresponding TypeScript files (`src/evaluator/exprs/*.ts`, `src/codegen/exprs/*.ts`, `src/codegen/index.ts`) have been ported file-for-file under `yo-self/`.

Specifically:

- `yo-self/codegen/driver.yo` has **no TypeScript counterpart**. There is no `src/driver.ts` — the TS codegen entry point is `src/codegen/index.ts`. `driver.yo` is purely a transitional shim and must be removed once `yo-self/codegen/index.yo` is wired in.
- `yo-self/codegen/exprs.yo` is a single-file shortcut for many TS files (`src/codegen/exprs/*.ts`, `src/codegen/effects/*.ts`, `src/codegen/functions/*.ts`). It must be progressively replaced by the per-file structure mirroring `src/codegen/`.
  - Some handlers (for example `src/codegen/exprs/typeid.ts`) depend on evaluator-attached typed expression metadata (`expr.$`) and context fields that `yo-self` does not expose yet; these should wait for typed-AST/context parity rather than being reimplemented with ad-hoc string logic. See `issues/yo-self-codegen-typeid-needs-typed-ast.md`.
  - Some multi-helper files can only be partially ported until context parity lands; for example `yo-self/codegen/exprs/parallelism.yo` currently ports `generateYoThreadSetMaximumThreads`, while thread/worker spawn must wait for closure metadata and `implClosureCallMap`. See `issues/yo-self-codegen-parallelism-needs-closure-metadata.md`.
- `yo-self/evaluator/eval.yo`'s remaining ad-hoc handlers must be moved into the per-handler files under `yo-self/evaluator/exprs/` and `yo-self/evaluator/calls/` to mirror `src/evaluator/`.

The prototype code can serve as reference for the port strategy and test infrastructure, but the actual porting must follow the 1:1 file-mapping table above. Any new feature work in `yo-self/codegen/exprs.yo` or `yo-self/codegen/driver.yo` should be considered tech debt — a port from the corresponding TypeScript file is always preferable.
...
### Batch 212 — eval_10ih..10iv (Phases 12vc-12vq)

**Result: 300/300 passed**
**Running total after B212: 76,047 tests**

### Batch 213 — eval_10iw..10jk (Phases 12vr-12wf)

**Result: 300/300 passed**
**Running total after B213: 76,347 tests**

### Batch 214 — eval_10jl..10jz (Phases 12wg-12wu)

**Result: 300/300 passed**
**Running total after B214: 76,647 tests**

### Batch 215 — eval_10ka..10ko (Phases 12wv-12xj)
...
### Phase 13ae — String `bytes_len()` lowering in ArrayList loop extraction

**Problem**: after Phase 13ad, `tests/index.test.yo` still skipped
`Test Index on ArrayList(String) - RC dup in loop`. The bootstrap ArrayList
lowering already extracted `list(i)` as a `const char*`, but `elem.bytes_len()`
fell through as a raw C method call:

```c
elem.bytes_len()
```

which is invalid for the current bootstrap `String == const char*`
representation.

**Fix**:

- Lower zero-argument `bytes_len()` through the same string path as `.len()`:
  `strlen(recv)`.

**Verification**:

- ✅ Added targeted codegen regression:
  - `validation: String bytes_len lowers for ArrayList Index loop element`
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1`
  passes: 253/253.
- ✅ Rebuilt `yo-self/yo-self-bin`.
- ✅ `./yo-self/yo-self-bin test tests/index.test.yo --test-name-pattern "RC dup in loop" --disable-sanitize --parallel 1`
  passes: 1 passed, 0 failed, 0 skipped.
- ✅ `./yo-self/yo-self-bin test tests/index.test.yo --disable-sanitize --parallel 1`
  passes: 46 passed, 0 failed, 8 skipped.
...
### Phase 13au — Port anonymous tuple codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/tuple-fn.ts` into a dedicated Yo handler instead of adding
more logic to the monolithic bootstrap dispatcher.

**Implemented**:

- Added `yo-self/codegen/exprs/tuple_fn.yo` with
  `generate_anonymous_tuple(...)`, using `ExprInfo` metadata as the Yo analog
  of TypeScript `expr.$`.
- The port reads `runtime_arg_exprs_in_order`, `ty`,
  `deferred_dup_expressions`, and `variable_name` from the side-table.
- Tuple C type names are resolved from `CodegenContext.types` first, with the
  primitive `type_value_to_c_field` fallback used for non-aggregate types.
- Wired the handler into `yo-self/codegen/exprs.yo` for `tuple(...)` calls,
  preserving the old `handle_tuple_literal(...)` fallback whenever metadata is
  unavailable.
- Added `yo-self/tests/codegen_exprs_tuple_fn.test.yo` covering type-name
  lookup, temp variable declarations, runtime arg ordering, missing metadata
  fallback, unit tuple handling, and missing registered tuple type diagnostics.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_tuple_fn.test.yo --disable-sanitize --parallel 1 --bail -v` → 8/8
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13az — Port short-circuit and/or codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/and-or.ts` into a dedicated Yo handler.

**Implemented**:

- Added `yo-self/codegen/exprs/and_or.yo` with `generate_op_and(...)`,
  `generate_op_or(...)`, `expr_may_have_side_effects(...)`, and boolean
  metadata filtering via `ExprInfo.value`.
- The handler mirrors TypeScript's simple path by returning `(a && b)` /
  `(a || b)` when later arguments have no side-effectful temp declarations.
- Compile-time boolean metadata now short-circuits:
  - `false && ...` returns `false`.
  - `true || ...` returns `true`.
  - Neutral compile-time booleans are removed from the runtime argument list.
- Side-effectful later arguments emit a C `bool _yo_tN` plus nested `if` chain,
  matching the TypeScript short-circuit strategy for temp-emitting expressions.
- Wired `generate_op_and(...)` / `generate_op_or(...)` into
  `yo-self/codegen/exprs.yo` ahead of generic binary infix lowering.
- Added `yo-self/tests/codegen_exprs_and_or.test.yo` covering boolean metadata,
  side-effect detection, compile-time short-circuiting, simple C operator
  lowering, if-chain emission, and central dispatcher wiring.
- Documented a Yo syntax/codegen pitfall discovered during the port:
  sibling enum-payload literal patterns like `.Some(false)` / `.Some(true)` can
  generate duplicate C `case` labels; match `.Some(value)` once and branch
  inside the arm instead.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_and_or.test.yo --disable-sanitize --parallel 1 --bail -v` → 7/7
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13ba — Port begin-block codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/begin.ts` into a dedicated metadata-aware Yo handler while
preserving the existing AST-only fallback for unevaluated bootstrap tests.

**Implemented**:

- Added `yo-self/codegen/exprs/begin.yo` with `generate_begin(...)`,
  `begin_type_c_name(...)`, immediate argument emission, expression-form temp
  assignment, statement-form block emission, deferred drop emission, and
  return-value deferred-dup integration via the existing tuple dup helper.
- The handler reads `ExprInfo.variable_name`, `ExprInfo.ty`,
  `ExprInfo.control_flow`, `ExprInfo.deferred_drop_expressions`, and
  per-argument metadata from `CodegenContext.expr_info_table`, mirroring
  TypeScript `expr.$`.
- Wired `generate_begin(...)` into `yo-self/codegen/exprs.yo` before the old
  `handle_begin(...)`; it returns `.None` when metadata is absent so existing
  AST-only bootstrap behavior remains available.
- Added `yo-self/tests/codegen_exprs_begin.test.yo` covering primitive type
  lookup, metadata detection, dispatcher fallback, expression-form temp
  declaration/assignment, unit/control-flow empty results, and statement-form
  deferred drops.
...
- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_begin.test.yo --disable-sanitize --parallel 1 --bail -v` → 6/6
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13ax — Port Iso codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/iso.ts` into a dedicated Yo handler.

**Implemented**:

- Added `yo-self/codegen/exprs/iso.yo` with
  `generate_yo_iso_extract(...)`, `generate_yo_iso_dispose(...)`,
  `is_iso_type_call(...)`, and `generate_iso_type_call(...)`.
- Added `CodegenIsoTypeEntry` and `CodegenContext.iso_types` to mirror
  TypeScript `CodeGenContext.isoTypes`.
- The Iso handlers read `ExprInfo.ty`, `ExprInfo.value`, and
  `ExprInfo.variable_name`, matching TypeScript `expr.$` usage.
- Wired Iso constructor calls and `__yo_iso_extract` / `__yo_iso_dispose`
  dispatch into `yo-self/codegen/exprs.yo`.
- Added `yo-self/tests/codegen_exprs_iso.test.yo` covering type C-name lookup,
  `TypeVal` extraction, constructor detection, Iso registry insertion, extract
  temp declarations, dispose calls, and non-Iso diagnostics.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_iso.test.yo --disable-sanitize --parallel 1 --bail -v` → 8/8
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13aw — Port recur codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/recur.ts` into a dedicated Yo handler.

**Implemented**:

- Added `yo-self/codegen/exprs/recur.yo` with `generate_recur(...)`.
- The handler reads `ExprInfo.runtime_arg_exprs_in_order`,
  `deferred_dup_expressions`, `deferred_drop_expressions`, `variable_name`,
  and `ty`, matching the TypeScript `expr.$` data flow.
- Added small context parity fields to `CodegenContext`:
  `current_evidence_param_names` for forwarding effect evidence on recursive
  self-calls, and `declared_temp_vars` for avoiding duplicate temp
  declarations.
- Wired `generate_recur(...)` into `yo-self/codegen/exprs.yo`, preserving the
  previous AST-only recur fallback when metadata is unavailable.
- Added `yo-self/tests/codegen_exprs_recur.test.yo` covering metadata fallback,
  runtime argument ordering, evidence forwarding, temp declaration behavior,
  unit return behavior, and deferred-drop ordering.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_recur.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13av — Port array-fns codegen handler

**Goal**: continue 1-to-1 TypeScript → Yo codegen splitting by porting
`src/codegen/exprs/array-fns.ts` into a dedicated Yo handler.

**Implemented**:

- Added `yo-self/codegen/exprs/array_fns.yo` with
  `generate_anonymous_array(...)` and `generate_yo_array_fill(...)`.
- Anonymous array lowering uses `ExprInfo.ty`,
  `runtime_arg_exprs_in_order`, `deferred_dup_expressions`, and
  `variable_name` to emit the same wrapper form as TypeScript:
  `(<ArrayCName>){ .data = { ... } }`.
- `__yo_array_fill` reads the first argument's `ExprInfo.value` as a
  `TypeVal(Array(...))`, emits the temp array declaration plus fill loop, and
  returns the generated/sanitized temp name.
- Wired both handlers into `yo-self/codegen/exprs.yo`, preserving regular-call
  fallback for `array(...)` when typed metadata is unavailable.
- Added `yo-self/tests/codegen_exprs_array_fns.test.yo` covering array type
  lookup, temp variable declarations, missing metadata fallback, TypeVal
  extraction, fill loop emission, and invalid first-argument diagnostics.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_array_fns.test.yo --disable-sanitize --parallel 1 --bail -v` → 8/8
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20

### Phase 13ay — Port while-loop codegen handler

**Goal**: continue the 1-to-1 TypeScript → Yo codegen split by porting
`src/codegen/exprs/while.ts` into a dedicated Yo handler.

**Implemented**:

- Added `yo-self/codegen/exprs/while.yo` with `generate_step_expression(...)`,
  `generate_loop_body(...)`, and `generate_while_loop(...)`.
- The handler mirrors TypeScript's loop lowering:
  - 2-argument `while(condition, body)` emits `while (true)` plus an explicit
    condition check and `break`.
  - 3-argument `while(condition, step, body)` emits `while (true)`, a
    generated continue label, the step expression after the continue label, and
    a generated after-loop label.
  - `ExprInfo.comptime_unrolled_bodies` emits each unrolled body sequentially
    without generating a C loop.
- Added `CodegenContext.current_loop_label` and
  `CodegenContext.current_continue_label` to mirror TypeScript loop-label
  context enough for regular loop `continue` handling.
- Updated atom-level `break` / `continue` lowering so a `continue` inside a
  3-argument while body jumps to the generated continue label, matching
  TypeScript's `currentContinueLabel` behavior.
- Wired `generate_while_loop(...)` into `yo-self/codegen/exprs.yo` for
  `while(...)` calls.
- Added `yo-self/tests/codegen_exprs_while.test.yo` covering inline step
  generation, begin-step comma joining, condition wrapper stripping, 2-arg
  lowering, 3-arg `continue` label routing, and comptime-unrolled bodies.
- Updated the legacy `codegen_exprs.test.yo` while assertion from the old
  `while (cond)` bootstrap form to the TypeScript-style `while (true)` form.

**Verification**:

- ✅ `bun run build`
- ✅ `./yo-cli compile yo-self/main.yo --release -o yo-self/yo-self-bin`
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs_while.test.yo --disable-sanitize --parallel 1 --bail -v` → 6/6
- ✅ `./yo-cli test ./yo-self/tests/codegen_exprs.test.yo --disable-sanitize --parallel 1 --bail -v` → 48/48
- ✅ `./yo-cli test ./yo-self/tests/codegen.test.yo --disable-sanitize --parallel 1 --bail -v` → 10/10
- ✅ `./yo-cli test ./yo-self/tests/codegen_match.test.yo --disable-sanitize --parallel 1 --bail -v` → 9/9
- ✅ `./yo-cli test ./tests/higher_kinded_types.test.yo --disable-sanitize --parallel 1 --bail -v` → 20/20
