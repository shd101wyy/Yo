...
| `src/codegen/exprs/*.ts`                            | `yo-self/codegen/exprs/*.yo`                            | 🔄 In progress (Phase 4Z-split + Phase 13at/13au/13av metadata-aware split: `expr.yo`, `gc.yo`, `sizeof.yo`, `consume.yo`, `async_completion.yo`, `parallelism.yo` (`__yo_thread_set_maximum_threads`), `inline_fns.yo` (operators, ptr/slice ops, randomness, sleep, noop, decr_rc, maybe_uninit_assume_init), `typeid.yo`, `panic.yo`, `open.yo`, `binding.yo`, `tuple_fn.yo`, and `array_fns.yo` ✅ ported and wired as 1-to-1; remaining handlers still inline in `exprs.yo`) |
