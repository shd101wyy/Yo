...
```
Yo source → Lexer → Parser → AST  (yo-self/lexer, yo-self/parser, yo-self/expr)
                              ↓
...
| Codegen — utils            | ~1100    | ⚠️ `yo-self/codegen/utils.yo` covers most pure helpers: sanitize, get_type_string(+`_opt`), get_variable_type_string, get_variable_name_for_codegen, get_enum_variant_c_name, should_avoid_const, can_optimize_as_simple_enum, can_optimize_as_nullable_pointer, get_deferred_dup/drop_target_atom_name, is_deferred_drop_for_closure_capture. Still pending: is_comptime_function, is_function_value_with_only_builtin_yo_inline_function_call (blocked on FunctionValue fields), is_comptime_only_struct_field (Struct field flag) |
...
| `closures.ts`                  | 320      | ⚠️ `closures.yo` (`check_variable_is_closure_captured` only)              | `codegen_exprs_closures.test.yo` (3 tests)        |
...
| `types/auto_derive.yo`                                       | 243   | Auto-derive logic that belongs in `types/utils.yo` (currently a stub).                                                                                                               |
