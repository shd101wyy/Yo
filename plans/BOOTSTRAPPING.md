...
| Codegen — utils            | ~1100    | ⚠️ `yo-self/codegen/utils/{index,fixup}.yo` + `codegen/constants.yo` cover 12/16 TS exports from `src/codegen/utils/index.ts` + the full `constants.ts` and `fixup.ts` files + 5 cross-handler line-shape formatters: `format_typed_var_name`, `indent_plus_two`, `indent_minus_two`, `format_indented_stmt`, `format_typed_init_line`, `format_typed_decl_line` — each consolidated from previously-duplicated per-handler helpers. Still pending (4): `findReturnedAsyncBlock` (async-runtime traversal), `getRuntimeStructFields` / `isComptimeOnlyStructField` (blocked on per-field `isCompileTimeOnly` flag on Struct), `isComptimeFunction` (blocked on `Func.return.isCompileTimeOnly`) |
