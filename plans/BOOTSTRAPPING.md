...
| Codegen — utils            | ~1100    | ⚠️ `yo-self/codegen/{utils,constants,fixup}.yo` cover 15/16 TS exports from `src/codegen/utils/index.ts` + the full `constants.ts` and `fixup.ts` files. Still pending: `findReturnedAsyncBlock` (async-runtime traversal), `getRuntimeStructFields` / `isComptimeOnlyStructField` (blocked on per-field `isCompileTimeOnly` flag on Struct), `isComptimeFunction` (blocked on `Func.return.isCompileTimeOnly`) |
| Doc system                 | ~1500    | ✅ ported (`yo-self/doc/`)                                                                                                                                                                                                                                                                                                                                                                                      |
| Build system               | ~1500    | ✅ ported (`yo-self/build_runner.yo` + `yo-self/build/build_registry.yo` divergent split)                                                                                                                                                                                                                                                                                                                       |
