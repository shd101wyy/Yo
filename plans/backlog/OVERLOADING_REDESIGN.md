...
`plans/archive/SLICE_REWORK.md` **deletes `String.as_str()`** (and `ArrayList.as_slice()`),
...
(`plans/archive/SLICE_REWORK.md` step "starts_with/ends_with/contains(str) overloads").
...
3. **Change std.** Rewrite `StrPattern` (§4) and the inherent `String` methods to the
   generic form. Update `plans/archive/SLICE_REWORK.md` to reflect that overloads are replaced
   by generics.
