...
- ~~Closure bodies (e.g. `Thread.spawn(io => ...)`) were skipped by the generation heuristic because `typeContainsSomeType(IO)` returns true through `IO`'s nested forall fields~~ **FIXED** by `a256da32` for the codegen-side skip. Closure bodies now reach `generateFunction`. The closure-using tests don't yet pass on their own because each has additional independent downstream issues (e.g. `shared.*` deref dispatch fails in the `assert` wrapper for Arc tests) tracked separately.

Test-suite status after these fixes: **1738/2508 passing** (up from 1253 immediately after `7b3b788b`, a +485-test improvement across this session). Remaining 770 failures cluster into the following orthogonal root causes:
