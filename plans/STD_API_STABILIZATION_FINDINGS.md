...
28. ~~Module-prefix stutter: base64_encode, hex_decode, json_parse, csv_parse, toml_parse, html_decode~~ **DONE 2026-09-11** — all eight `std/encoding` modules de-stuttered (`percent`/`utf16` too), old spellings kept as deprecated aliases removed in v0.2.32; see the §4 Encoding record in `STD_API_STABILIZATION.md`. `glob_match` is NOT done: `glob.match` is unspellable (`match` is a keyword) and `std/fs`'s own exported `glob()` collides with the module binding — written up in `plans/backlog/MODULE_PREFIX_STUTTER_REMAINDER.md`, which also lists the other ~20 std modules with the same shape. `csv_write→stringify` was deliberately NOT taken (a prefix removal is not a vocabulary change); `json_stringify_pretty` became `json.stringify_pretty`.
...
16. HTTP: ~~status bare i32 (http_status_text 13 codes http.yo:133-149); headers ArrayList(HttpHeader), no remove/get_all; body String (http.yo:156) — parse_response reassembles body via string concat (http.yo:290-300) so binary bodies broken client-side (parse_request fixed at :313)~~ **FIXED 2026-09-09**: `StatusCode` (u16 newtype, class predicates, 34 reason phrases, 22 named codes), `HeaderMap` (case-insensitive, insertion-ordered multimap with `get_all`/`remove`/`insert` vs `append`), and `parse_response` slices the body as BYTES on the CRLFCRLF boundary — which also fixed an out-of-range status being accepted and a missing reason phrase being rejected. STILL OPEN: Connection: close forced (client.yo:171, server.yo:95); only first DNS answer tried (client.yo:191).
