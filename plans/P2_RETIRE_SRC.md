...
| unsafe-gate.test.ts (126)                  | unsafe() gate outside std/yo-self                | **DONE 2026-08-12.** Positive arm → cli-case `unsafe-pragma-ok` (build-run, pins `v = 42`); signature gate → `ptr-type-safe-code`; the body-level negatives → `addr-of-safe-code` + `unsafe-call-safe-code`, both PASS. Those two were blocked on the def-eval swallow (self accepted, TS rejected) and are unblocked by the marker work: the privilege gate still throws inside main's body and the swallow still eats it, but the statement it drops leaves a `// Failed to transpile` marker in `__yo_user_main`, which is now fatal — so rc parity is restored without un-swallowing anything. See issues/fixed/self-hosted-compile-swallows-undefined-call.md                                                                                                                                                                                                                               |
