...
Written at hand-off. Read this, then `plans/archive/STD_API_AUDIT.md` (the campaign) and
`plans/archive/STD_API_AUDIT_HANDOVER.md` (the older, still-valid method notes).

The standing instruction driving all of it: **finish `plans/archive/STD_API_AUDIT.md` and
...
compatibility; `plans/reference/TARGET_TRIPLES.md` supersedes §5.4's "what does NOT
...
| row | what | status |
| --- | --- | --- |
| C36 | dispatch-mode cond: a sibling arm's SECOND await was never awaited/extracted (chained layer with no binding; `=` re-assignment targets not recorded) — plain `http://` fetch crashed since #322 | FIXED, issues/async-cond-dispatch-skips-chained-sibling-arm.md |
...
| C39 | `Exception(throw: …)` handler assigning a captured `Box` fails inference | CLOSED (by design; diagnostic fixed 2026-08-29), issues/fixed/exception-handler-closure-with-box-capture-fails-inference.md |
...
From `plans/archive/STD_API_AUDIT.md` §2. Each has an issue file with a reproducer.
...
maintainer. `plans/reference/RELEASE_ASSET_TRIPLES.md` has the mapping table and the four
