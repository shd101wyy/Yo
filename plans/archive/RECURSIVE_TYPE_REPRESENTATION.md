...
- **Stage 0 (PREREQUISITE):** re-run the full-compile swallow + innermost-expr instrumentation
  (function*type.yo `_trial_eval_fn_body` handler + expr.yo `evaluate_expression` current-expr
  ...
  (`awk '/^static .\*yo_id*[0-9]+\(/'` on stage2.c). This re-prioritizes against the ACTUAL
