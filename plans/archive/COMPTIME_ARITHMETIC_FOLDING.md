...
2. **`function.ts:1630-1657`** (the priority logic the maintainer flagged):
   _"Comptime function call has higher priority than normal function call …
   so we eagerly evaluate the call that can be done at compile-time."_
   ...
   tiebreak (1664-1681) prefers comptime _parameter_ types (e.g.
...
- ✅ Dispatch to it: `_expr.yo:726-727` routes `is_comptime_numeric_fn_call(expr)`
  → the folder. So a _direct_ `__yo_comptime_usize_add(...)` call is handled.
...
- ✅ Infix dispatch scaffolding: `function.yo:315-386` already resolves
  _comparison_ operators via `get_receiver_methods_by_name_from_env` +
...
bool-_typed_ UnknownVal, not a concrete bool. To actually FOLD arithmetic to
