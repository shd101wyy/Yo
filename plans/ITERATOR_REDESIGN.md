...
`inout(name) : T` is already a binding-kind modifier on a parameter: the binding refers to caller-side storage. This proposal extends the same modifier into the **top of a return slot**, with two crucial restrictions:

- `inout` in a return slot appears only at the **outermost** position. `inout(T)` is not a type expression — `Option(inout(T))`, `(inout(T), bool)`, `struct(x : inout(T))` are all syntactically rejected.
- A call to a function whose return slot is `inout(T)` produces a **place expression** in C-ABI terms (a `T*`) but a **inout-bindable expression** in the evaluator. It can flow only into one of:
  - The right-hand side of an `inout(name) := expr;` binding form (new local binding kind).
  - The argument slot of another function's `inout` parameter (chaining).
  - An immediate read (`x := f()` where `f()` returns `inout(T)` does an auto-deref-copy, same shape as reading a parameter `inout` binding).
  - An immediate write (`f() = v;` writes through the projection — same as `arr(i) = v;` today via the Index trait).
  - An immediate field access whose own type is `inout(T')` (chaining through nested projections).

The structural restriction means: any path that would smuggle `inout(T)` into a stored binding, a struct field, or a closure capture is a compile error at the construction site. The user can't write `r := some_call()` and have `r` be "an inout reference" — either `some_call()` is auto-derefed (producing a `T` value, copied) or the LHS is the new `inout(name) := ...` binding form.

### The new binding form: `inout(name) := projection_call(...);`

```rust
inout(x) := list.project(pos);
...
The `inout(name) :=` form is new syntax — added to the parser alongside the existing `name := value` and `(name : Type) = value` forms. The bound variable carries `isInout: true` in the evaluator env, so the existing `inout`-aware codegen (binding lowers to `T*`, reads emit `(*x)`, writes emit `(*x) = v`) applies.
...
```rust
project : fn(inout(self) : Self, pos : Position) -> inout(Element);
```
...
Iterator :: trait(
  ...
  next : fn(inout(self) : Self) -> Option(Self.Position),
...
Indexable :: (fn(comptime(Idx) : Type) -> comptime(Trait))(
  trait(
    Element : Type,
    project : fn(inout(self) : Self, pos : Idx) -> inout(Self.Element)
  )
);
```
...
impl(
  ...
  Indexable(usize)(
    Element : T,
    // Body computes the address of the element; the `inout(T)` return
    // slot tells the call site to receive it as an inout-binding.
    project : (fn(inout(self) : Self, pos : usize) -> inout(T))({
      match(
        self._ptr,
        .Some(p) => unsafe((p &+ pos)),
...
A function whose return slot is `inout(T)`:
...
The C ABI of `inout(T)`-return is identical to a raw-pointer return. The evaluator simply tracks the "this is a projection, not a freestanding pointer" attribute and forbids the result from flowing into non-projection contexts.
...
### Soundness of `inout(T)` return slots — the flowability rule
...
- **R1.** A name reference is flowable iff its binding has `isInout: true` (an `inout`-typed parameter, or a local declared via `inout(name) := ...`).
- **R2.** `expr.field` is flowable iff `expr` is flowable. (A field-of-borrow is itself a borrow.)
- **R3.** `expr(args)` is flowable iff the callee's return slot is `inout(T)` and every `inout`-typed argument it receives is itself flowable.
- **R4.** `cond` / `match` arms are checked independently — every arm reachable as a return value must be flowable.

The evaluator runs this check on the return expression of every `inout(T)`-returning function, AND on the RHS of every `inout(name) := expr;` local binding. The same predicate covers both because in both positions we are creating an `inout`-bindable handle that must point at live storage.
...
(fn() -> inout(i32))({
  list := ArrayList(i32).new();
  inout(r) := list.project(usize(0));  // R3: list is local → reject HERE
  return r;
});

// Chain through an inout-bound local — accept.
(fn(inout(p) : Point) -> inout(i32))({
  inout(r) := p.x;   // R2: p flowable → r is inout-bound and flowable
  return r;          // R1: r.isInout, initializer flowable → accept
});

// Two inout parameters, conditional choice — accept (both arms flowable).
(fn(inout(p) : Point, inout(q) : Point, use_p : bool) -> inout(i32))(
  cond(
    use_p => return p.x,
    true  => return q.x
  )
...
(fn(inout(p) : Point, fallback : i32) -> inout(i32))(
  cond(
...
impl(Point,
  get_x : (fn(inout(self) : Self) -> inout(i32))(self.x)
);
(fn(inout(p) : Point) -> inout(i32))(
  p.get_x()  // R3: get_x returns inout(i32); its inout-self arg = p (R1) → accept
);

// Chained projection — each link in the call graph re-validates.
get_inner :: (fn(inout(p) : Outer) -> inout(Inner))(p.inner);
get_x     :: (fn(inout(i) : Inner) -> inout(i32))(i.x);
(fn(inout(p) : Outer) -> inout(i32))(
...
get_first :: (forall(T : Type), fn(inout(arr) : ArrayList(T)) -> inout(T))(
  arr.project(usize(0))  // R3: project returns inout(T); arg = arr (R1) → accept
);
...
// inout(name) := from a function returning a value — REJECT at the binding.
fetch_value :: (fn() -> i32)(i32(42));
(fn() -> unit)({
  inout(r) := fetch_value();  // RHS has type i32 (not inout(i32)) → reject
});
...
**What the rule explicitly forbids.** Rebinding an `inout`-bound local — `inout(r) := new_projection` after `inout(r)` is already declared — is not in v1. Each `inout(name) := expr;` is a fresh binding; reassigning the binding (as opposed to writing _through_ it via `r = newValue`) is rejected. This keeps the soundness argument from having to reason about mutating origins.

### Other structural guarantees (summary)

In addition to the flowability rule above, the language enforces:

- `inout(T)` cannot appear inside any other type expression — guaranteed by parsing `inout(T)` only at the top of a return slot or a parameter slot. So `Option(inout(T))`, `struct(x : inout(T))`, `(inout(T), bool)`, generic args holding `inout(T)`, etc. all reject at parse time.
- An `inout`-bound binding (parameter or `inout(name) := ...` local) cannot be captured by a closure — already enforced by the Phase B gate from `plans/MEMORY_SAFETY.md` (`Cannot capture inout parameter 'x' in a closure`). The same `isInout: true` flag covers both binding sources.
...
- An `inout`-bound local cannot be returned from a function unless the function's return slot is `inout(T)` AND the expression satisfies the flowability rule.
...
### Phase A — Parser & evaluator support for `inout(T)` return slot

- Parse `-> inout(T)` in function signatures. Annotate the parsed `FunctionParameter`-style return descriptor with `isInout: true`.
- Reject `inout(T)` anywhere inside a type expression that is not the top of a return slot. Error message: `'inout(T)' is a binding-kind marker — it can only appear in a parameter slot or at the top of a return type. To express borrow-typed storage, redesign the API to expose a projection method instead.`
- Codegen: function returning `inout(T)` emits `T*` as its C return type. The body must produce a `T*` value (typically via an `unsafe(p &+ idx)` or `&(self.field)` expression in a privileged file).
- Tests: positive cases (a function returning `inout(T)` compiles, call site can chain into another `inout` parameter); negative cases (`Option(inout(T))`, `struct(x : inout(T))`, `(inout(T), bool)` all rejected with the targeted error).

### Phase B — `inout(name) := expr;` local binding form

- Parser: recognize `inout(name) := expr;` as a binding declaration whose target carries `isInout: true`.
- Type-check: the RHS must be a call whose return slot is `inout(T)`, or the access of an `inout`-bound parameter, or a projection chain. Otherwise: `'inout(name) := ...' requires an inout-yielding right-hand side. The expression on the right is of type ${T}, not an inout projection.`
...
- The `for` macro in `std/prelude.yo` changes its expansion to insert the `inout(x) := coll.project(pos);` binding between the `next()` call and the body.
...
```rust
next : fn(inout(self) : Self) yields inout(Item)
```
...
3. **HashMap key/value tuple iteration.** Current API yields `IterPair(K, V)` by value. Migration target: yield positions whose projection returns a tuple of `(inout(K), inout(V))`? Tuple-of-inouts is a corner case for the structural rule — `(inout(K), inout(V))` is a tuple-type containing `inout`, which the rule rejects. The workaround is to projection-return a single value of type `struct(key : K, value : V)` and let the user destructure inside the loop, OR special-case the for-macro to handle two projections in parallel. Open.

4. **Interaction with effect-typed `ctl` bodies.** A `ctl(...) -> R` function can `unwind` out of an `inout(x) := projection_call(...);` block. What happens to the projection? Lean: nothing — the projection has no destructor, it just dangles when the call frame is gone, which is fine because nothing was holding a reference to it after unwind.
