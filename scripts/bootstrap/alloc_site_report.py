...
if deep:
    by5 = collections.Counter()
    for r in rows:
        w = int(r[6]) if (weight_cap and len(r) > 6) else int(r[1])
        chain = [nm(r[4]), nm(r[5])] + [nm(a) for a in r[7:10]]
        by5[(r[2], " <- ".join(chain))] += w
    print("== by (type, 5-frame chain)")
    for k, v in by5.most_common(top):
        print("%10d  %s  %s" % (v, k[0], k[1]))
