...
- **A new build builtin cannot be wrapped by `std/build.yo` in the same release that adds it.** A module-level `::` binding there is forced by its own `export(...)`, so an older compiler fails the WHOLE module (`error[E0401]: Variable "__yo_build_x" not found`) — and `fixpoint-arm64.yml` bootstraps gen-1 by running `yo build` with the SEED, which evaluates `std/build.yo` every time. So the builtin lands first (build files call it directly), and the friendly surface follows once `SEED_VERSION` carries it — the generation-A/B split recorded in `plans/backlog/SEED_VERSION_AUTOMATION.md`. Worse, a pre-v0.2.31 seed does not REPORT the failure: it still swallows build-file evaluation errors and prints `No build steps defined.`, so a broken `std/build.yo` looks like an empty build.
