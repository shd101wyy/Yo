...
| `plans/SELF_HOSTING_COMPLETION.md`           | The self-hosting roadmap: P1 CLI parity (**COMPLETE**), P2 retire `src/`+bun (**COMPLETE**), P3 release bundles + install scripts (Koka model), P4 LSP + VS Code                                              |
...
| `plans/P4_LSP.md`                            | The LSP + VS Code phase — IN PROGRESS since 2026-08-22: `yo lsp` serves stdio LSP (full sync + push diagnostics, `src/lsp/`), the extension carries a plain-JS client (`yo.binPath` setting); slices 3-6 open |
...
- **The VS Code extension bundles an LSP client since 2026-08-22** (plain JS, no build step): it spawns `yo lsp` (configurable via `yo.binPath`) for push diagnostics. Hover/completion/definition are NOT served yet — those are P4 slices 3-5 (`plans/P4_LSP.md`). With `yo.lsp.enabled: false` (or no yo binary) it degrades to syntax highlighting.
