...
| `plans/LAZY_TOPLEVEL_BINDINGS.md`         | P0–P4 LANDED 2026-09-05: order-independent `::` definitions and `impl` registration within a module — pending entries forced on a lookup MISS (two-phase fn forcing, cycle errors, error attribution notes); the impl-block forward shells/thunks are retired in favour of the same forcing (P3); P5 (seed bump, then `std/`/`src/` may use forward refs) remains; user docs `docs/*/DEFINITION_ORDER.md` |
