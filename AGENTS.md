...
| `src/manifest.yo`                        | `yo.toml` — the package manifest: parser, discovery (nearest manifest above a file), and the `import("name")` closure every command resolves through (plans/BUILD_AND_DEPENDENCY_SYSTEM_REDESIGN.md §4.1/§4.5.1) |
...
| `src/fetch.yo`                           | Cache primitives: `fetch_package` (commit decision, integrity check, clone), content hashing, the `.yo-content-hash` sidecar                                                                                    |
...
- **`yo install` auto-prunes stale lock entries.** When a dep is removed from `yo.toml` (`yo remove`), the next `yo install` drops it from `yo.lock`. The global cache is not auto-cleaned. `yo.lock` is **v2** (`version = 2`, `[[package]]` entries with `source = "git+url#ref"`/`"path+rel"`, `commit`, `integrity`, `dependencies`); a v1 lock is not read — the next `yo install` regenerates it. One version per dependency NAME across the whole graph: the resolver (`src/resolver.yo`) unifies every requirer's range/pin into one ref or fails naming the requirers.
- **Dependencies live in `yo.toml`, not in `build.yo`.** `build.dependency("name")` only REFERENCES a manifest entry (the runner rejects an undeclared name); `deps.yo`, the struct forms `build.dependency({…})`/`build.path_dependency({…})`, `add_import`/`add_import_list`, `yo fetch` and `yo install <spec>` are gone (plans/BUILD_AND_DEPENDENCY_SYSTEM_REDESIGN.md §4.1, landed 2026-09-12). `import("dep")` resolves in EVERY command through the nearest `yo.toml` above the file (`src/manifest.yo`), so `yo check`/`yo test`/LSP need no build.
