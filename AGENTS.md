...
| `plans/LAZY_TOPLEVEL_BINDINGS.md`         | PROPOSED 2026-09-02: lift the no-forward-reference rule — order-independent `::` definitions and `impl` registration via pending bindings forced on first reference (two-phase fn forcing, cycle errors, module-end forcing); P0 is a check-time "forward reference" diagnostic replacing today's silent hollow stub; acceptance = byte-identical C on the existing corpus |
