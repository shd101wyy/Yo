...
| `plans/backlog/FIXED_REGION_ALLOCATOR.md`     | BACKLOG (2026-09-09): third `--allocator` choice `fixed` — a TLSF general-purpose allocator over ONE static `.bss` region sized by `--heap-size`, emitted inline (no new link input); P0 fixes the unchecked-NULL RC constructors (OOM is UB today); `--debug-heap` live-at-exit leak oracle; §6 lists the remaining embedded/freestanding blockers it composes with |
