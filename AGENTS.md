...
| `src/install_command.yo`                 | `yo add` / `yo remove` / `yo install` / `yo update` — semver ranges over git tags, ref resolution, fetch + `yo.lock`                                                                                  |
...
- **`yo install` auto-prunes stale lock entries.** When a dep is removed from `yo.toml` (`yo remove`), the next `yo install` drops it from `yo.lock`. The global cache is not auto-cleaned.
