...
✅ **Phase 6g C compiler utilities.** `yo-self/compiler-utils/compiler_utils.yo` mirrors `src/compiler-utils.ts`. `CompilerInfo` object captures compiler name, MSVC/Windows/emcc flags. `SanitizerKind` enum (`Address`/`Leak`). `get_compiler_info(compiler)` detects MSVC/GCC-on-Windows/Clang-on-Windows/emcc from the command name. `get_sanitizer_flags(sanitize, info)` dispatches on `SanitizerKind` via `match` and per-platform `cond` to return a `SanitizerFlags(flags, warning, info)`. `get_macos_lsan_suppressions()` returns the LSAN suppression string on macOS. `check_compiler_available(cc, io, exn)` runs `cc --version` and returns `bool`. `find_available_compiler(io)` tries `clang`/`cc`/`gcc`/`zig`/`cl` with a per-candidate `given(try_exn)` handler to swallow spawn failures. `is_liburing_available(io, exn)` runs `pkg-config --exists liburing`. 9 unit tests; 426 yo-self tests pass.
...
- **Total: 426 tests passing** under `yo-self/tests/`
