...
local handle_error_message = function(r)
    if type(r) == "string" and
        (r:match("attempt to yield from outside a coroutine") or r:match("cannot resume dead coroutine")) then
        ...
        local continue_ = function(co_)
            return function(...)
                ...
                if not is_eff_obj(r) then
                    ...
                    return resend(r.eff, r.arg, function(...)
                        return resume(co, ...)
                    end)
                end
            end
